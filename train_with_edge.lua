...
cmd:option('-edge_loss_weight', 1.0)
...
cmd:option('-backend', 'cuda', 'cuda|opencl')
 ...
 function main()
  ...
  local function f(x)
    ...
    --print 'Now compute the perceptual loss and gradient'
    local percep_loss = 0
    if percep_crit then
    ...
    local loss = pixel_loss + percep_loss + edge_loss
  ...
  for t = 1, opt.num_iterations do
    ...
    if t % opt.checkpoint_every == 0 then
      ...
      for j = 1, val_batches do
        ...
        val_loss = val_loss + pixel_loss + percep_loss + edge_loss
      end
