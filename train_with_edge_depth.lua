...
require 'fast_neural_style.EdgeCriterion'
...
cmd:option('-edge_loss_weight', 1)
cmd:option('-depth_loss_weight', 1)
...
cmd:option('-style_image', 'images/styles/wave.jpg')
...
-- Options for edge reconstruction loss 
cmd:option('-edge_proto_file', 'edge_detection/hed_deploy.prototxt')
cmd:option('-edge_model_file', 'edge_detection/hed_pretrained_bsds.caffemodel')
cmd:option('-edge_layers', '31')
cmd:option('-edge_weights', '100.0')

-- Options for depth reconstruction loss
cmd:option('-depth_network', 'depth_prediction/..')
cmd:option('-depth_layers', '5')
cmd:option('-depth_weights', '100.0')
...
cmd:option('-backend', 'cuda', 'cuda|opencl')
 ...
 function main()
  ...
  opt.edge_layers, opt.edge_weights =
    utils.parse_layers(opt.edge_layers, opt.edge_weights)
    

  print (opt.edge_layers)
  print (#opt.edge_layers)

  print (opt.depth_layers)
  print (#opt.depth_layers)
  ...
  -- Set up the edge loss function
  local edge_crit
  if opt.edge_loss_weight > 0 then
    local edge_loss_net = loadcaffe.load(opt.edge_proto_file, opt.edge_model_file, 'nn'):type(dtype)
    local crit_args = {
      cnn = edge_loss_net,
      edge_layers = opt.edge_layers,
      edge_weights = opt.edge_weights
    }

    edge_crit = nn.EdgeCriterion(crit_args):type(dtype)
  end
  ...
  local function f(x)
    ...
    --print 'Now compute the pixel loss'
    local pixel_loss = 0
      if pixel_crit then
    ...
    print 'Perceptual loss is:'
    print (percep_loss)

    
    -- Compute Edge loss and gradient
    local edge_loss = 0
    if edge_crit then
      local target = {content_target=y}
      edge_loss = edge_crit:forward(out, target) -- may need to edit target
      edge_loss = edge_loss * opt.edge_loss_weight
      local grad_out_edge = edge_crit:backward(out, target)
      if grad_out then
        grad_out:add(opt.edge_loss_weight, grad_out_edge)
      else
        grad_out_edge:mul(opt.edge_loss_weight)
        grad_out = grad_out_edge
      end
    end
    print 'Edge loss is:'
    print (edge_loss)
    ...
    if depth_crit then
      local target = {content_target = y}
      depth_loss = depth_crit:forward(out, target)
    ...
    local loss = pixel_loss + percep_loss + edge_loss + depth_loss
  ...
  if opt.task == 'style' then
    ...
    for i, k in ipairs(opt.edge_layers) do
      style_loss_history[string.format('edge-%d', k)] = {}
    end
  ...
  for t = 1, opt.num_iterations do
    ...
    if opt.task == 'style' then
      ...
      for i, k in ipairs(opt.edge_layers) do
        table.insert(style_loss_history[string.format('edge-%d', k)],
          edge_crit.edge_losses[i])
      end
      for i, k in ipairs(opt.depth_layers) do
    ...
    if t % opt.checkpoint_every == 0 then
      ...
      for j = 1, val_batches do
        ...
        local edge_loss = 0
        if edge_crit then
          edge_loss = edge_crit:forward(out, {content_target=y})
          edge_loss = opt.edge_loss_weight * edge_loss
        end
        local depth_loss = 0
        if depth_crit then
        ...
        val_loss = val_loss + pixel_loss + percep_loss + edge_loss + depth_loss
      end
